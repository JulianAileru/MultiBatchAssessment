...
logging.basicConfig(
    ...
    datefmt='%Y-%m-%d %H:%M:%S',
    filename='BatchEffectsDashboard.log'
)
...
class Preprocessor:
    ...
    def apply(self,data):
        ...
        if self.imputation_method == "Global Minimum Value":
            data = data.fillna(data.min().min() * .10)
...
class SERRF(BatchCorrector):
    def __init__(self,blank_str,qc_str,n_jobs=-1):
        self.rowvar = False
    ...
    def adjust_data_labels(self,data,metadata):
        ...
        return data,metadata
    def compute_correlation(self,current_batch):
        """
        Current Batch is of shape (n_signals,n_samples)
        """
        train = current_batch.loc[:, current_batch.columns.str.endswith("_QualityControl")]
        target = current_batch.loc[:, current_batch.columns.str.endswith("_Biological")]
        ...
        for signal in current_batch.index:
        ...
        return None
    def top_correlated(self,current_batch,n=10):
        """
        ...
        for signal in current_batch.index:
            ...
            while n <= max_len:
                temp_train = set(train_lst[:n])
                temp_target = set(target_lst[:n])
    ...
    def fit_predict(all_data,current_batch,features,signal):
        ...
        """
        train_id = current_batch.columns[current_batch.columns.str.endswith("_QualityControl")].tolist()
        test_id = current_batch.columns[current_batch.columns.str.endswith("_Biological")].tolist()
        ...
        train_scale_factor = train_pred_original_scale / all_data.loc[signal, all_data.columns.str.endswith("_QualityControl")].mean()
        norm[train_id] = norm[train_id] / train_scale_factor
        
        # Adjust QC to match global median
        norm[train_id] = norm[train_id] / (norm[train_id].median() / all_data.loc[signal, all_data.columns.str.endswith("_QualityControl")].median())
        ...
        test_scale_factor = test_pred_centered / all_data.loc[signal, all_data.columns.str.endswith("_Biological")].median()
        norm[test_id] = norm[test_id] / test_scale_factor
        
        # Adjust biological samples to match global median
        norm[test_id] = norm[test_id] / (norm[test_id].median() / all_data.loc[signal, all_data.columns.str.endswith("_Biological")].median())
        ...
        if outliers.any():
            ...
            attempt = ((current_batch.loc[signal, test_id]) - 
                    ...
                    all_data.loc[signal, all_data.columns.str.endswith("_Biological")].median())
        ...
        return norm
    def normalize_all_batches(self,data,normalized_data):
        result = normalized_data.copy()
        qc_cols = data.columns[data.columns.str.endswith("_QualityControl")]
        sample_cols = data.columns[data.columns.str.endswith("_Biological")]
        for signal in data.index:
            # Calculate the correction factor
            sample_median_norm = result.loc[signal, sample_cols].median()
            qc_median_all = data.loc[signal, qc_cols].median()
            sample_median_all = data.loc[signal, sample_cols].median()
            sample_std_all =  data.loc[signal, sample_cols].std()
    ...
    def final_fix(self,result):
        normed_target = result.loc[:,result.columns[result.columns.str.endswith("_Biological")]]
        normed_train = result.loc[:,result.columns[result.columns.str.endswith("_QualityControl")]]
        for signal in result.index:
            ...
            if not valid_vals.empty:
                ...
                std_ = valid_vals.std()
                std_ = std_ * 0.1 if std_ > 0 else 1e-8
    ...
    def correct(self,_data,_metadata):
        ...
        if self.rowvar:
            _data = _data.T
            self.logger.info(f"Transposing data: {self._data.shape}")
        data = _data.copy()
        metadata = _metadata.copy()
        self.logger.info('Adjusting Sample Name Information')
        data,metadata = self.adjust_data_labels(data=data,metadata=metadata)
        self.logger.info(f'Number of QC Samples {len(data.index.str.endswith("_QualityControl"))} \n Number of Biological Samples {len(data.index.str.endswith("_Biological"))} \n Number of Blanks {len(data.index.str.endswith("_BLANK"))}')
        data = data.T
        signals = data.index.to_list()
        self.logger.info('Applying SERRF Correction')
        self.logger.info("Initializing Models")
        normalized_batches = []
        batches = metadata['batch'].unique()
        for batch in batches:
            current_batch = data.T.groupby(metadata['batch']).get_group(batch).T
            self.compute_correlation(current_batch=current_batch)
            self.top_correlated(current_batch,n=self.num_features)
            normalized_signals = []
            normalized_signals = Parallel(n_jobs=self.n_jobs)(delayed(SERRF.fit_predict)(all_data=data,current_batch=current_batch,
                                                                                features=self.features,signal=signal) 
                                                                                for signal in tqdm(signals,desc=f'\t Buliding models for batch {batch}',file=logfile))
            # # For debuging per signal
            # for signal in tqdm(signals,desc=f'Normalizing batch {batch}'):
            #     norm_signal = self.fit_predict(all_data=data,current_batch=current_batch,features=self.features,signal=signal)
            ...
            self.logger.info(f"Batch {batch}: COMPLETE")
        normalized_batches = pd.concat(normalized_batches).T
        self.logger.info('Normalizing All Batches')
        result = self.normalize_all_batches(data=data,normalized_data=normalized_batches)
...
class QC_SVRC(BatchCorrector):
    ...
    def qc_svrc(self,_data,_metadata):
        ...
        for idx,batch in group_by_batch:
            ...
            results = Parallel(n_jobs=self.n_jobs)(delayed(QC_SVRC.svr_function)(QC[col],Bio[col],qc_injection_order,bio_injection_order,qc1) for col in tqdm(QC.columns,desc=f'Correcting signals...'))
...
class QC_RFSC(BatchCorrector):
    """
    Data is expected to be in log space()
    """
    def __init__(self,qc_str,blank_str,n_jobs=-1):
        self.logger = logging.getLogger(self.__class__.__name__)
        self.qc_str = qc_str
    ...
    def qc_rfsc(self,_data,_metadata):
        ...
        for idx,batch in group_by_batch:
            ...
            bio_injection_order = metadata.loc[Bio.index,'injection_order']
            self.logger.info(f'Batch: {idx}\nNumber of QC Samples: {QC.shape[0]}\nNumber of Biological Samples: {Bio.shape[0]}')
            results = Parallel(n_jobs=self.n_jobs)(delayed(QC_RFSC.rfr_function)(QC[col],Bio[col],qc_injection_order,bio_injection_order) for col in tqdm(QC.columns,desc=f'Correcting signals...'))
            lst.append(pd.concat(results,axis=1))
...
class BatchCorrectionPipeline:
    ...
    def original_sample_names(self,corrected_data):
    ...
    def correct(self, data, metadata):
        ...
        return self.original_sample_names(corrected_data)
...
#     preprocessing_config=Preprocessor(imputation_method=None,log_transform=True)
# )

# pipeline
# results = pipeline.correct(data,metadata)
# pca_plot(results,metadata,hues=['sample_type'])