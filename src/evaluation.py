...
class Evaluation(EvaluationInfo):
    ...
    def pvca(self,explained_variance=.50,imputation_method='Global Minimum Value',normalization_method=None,transformation_method=None,ignore_blanks=True):
        var_comp = make_rfunc("""
                      function(model){
...
# test = Evaluation(data=pd.read_csv('/Users/jaileru/GitHub/batch-effects-dashboard/data/streamlit_sample_data.csv'),
...
# corrected = test.Evaluation_correction(between_batch=True)
# Evaluation.pca_plot(D=corrected,M=test.metadata,pca_hue='sample_type')
# # test
# # Evaluation.pca_plot(D=test.D,M=test.M,pca_hue='sample_type')
...
# #Evaluation.pvca(data=test.data,metadata=test.metadata)
# print(df)